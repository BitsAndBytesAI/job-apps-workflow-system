...
def start_resume_generation_run(
    *,
    ...
    thread = Thread(target=_execute_resume_generation, args=(run["id"], payload), daemon=True)
    thread.start()
    return run


def _execute_resume_generation(run_id: str, payload: dict[str, Any]) -> None:
